...
export function Activities() {
  ...
  return <div className="screen-enter">
    ...
    <section className="scheduled-activities surface"><header><div><span className="eyebrow">PROGRAMMATE</span><h3>Prossime attività</h3></div><small>Le attività visibili alla famiglia alimentano anche la loro timeline.</small></header>{state.scheduledActivities.map((item) => <div key={item.id}><CalendarStamp date={item.date} /><div><strong>{item.title}</strong><small>Elena Bianchi · {item.time} · {item.owner}</small></div><span className="status-pill status-confermata">{item.status}</span>{item.visibleToFamily && <span className="family-visible">Visibile alla famiglia</span>}</div>)}</section>
    ...
    <ActivityBuilder open={builderOpen} onClose={() => setBuilderOpen(false)} onStart={() => { setBuilderOpen(false); navigate("/interazione/elena"); }} onSchedule={(form) => { actions.scheduleActivity({ activityId: "custom", title: form.type, residentId: "elena", date: "2026-09-22", time: form.time, visibleToFamily: form.visibleToFamily }); setBuilderOpen(false); }} />
    <GameOverlay game={game} onClose={() => setGame(null)} />
  </div>;
}

function ActivityBuilder({ open, onClose, onStart, onSchedule }) {
  const [form, setForm] = useState({ type: "Ricordi guidati", duration: "15", topic: "Viaggi", resident: "Elena Bianchi", level: "Leggera", group: "Individuale", memory: "Viaggio a Palermo", time: "10:30", visibleToFamily: true });
  return <Modal open={open} title="Crea o programma attività" onClose={onClose} size="lg"><div className="builder-grid"><form className="form-grid">{Object.entries({ type: "Tipologia", duration: "Durata", topic: "Argomento", resident: "Ospite", level: "Complessità", group: "Modalità", memory: "Memoria da utilizzare" }).map(([key, label]) => <label key={key}>{label}<select value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })}>{key === "type" ? ["Ricordi guidati", "Conversazione libera", "Fotografie", "Musica"].map((v) => <option key={v}>{v}</option>) : key === "duration" ? ["10", "15", "20"].map((v) => <option key={v} value={v}>{v} minuti</option>) : key === "level" ? ["Leggera", "Media"].map((v) => <option key={v}>{v}</option>) : key === "group" ? ["Individuale", "Gruppo"].map((v) => <option key={v}>{v}</option>) : <><option>{form[key]}</option><option>Nessuna preferenza</option></>}</select></label>)}<label>Orario domani<input type="time" value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} /></label><label className="check-row"><input type="checkbox" checked={form.visibleToFamily} onChange={(e) => setForm({ ...form, visibleToFamily: e.target.checked })} /> Mostra alla famiglia come iniziativa della struttura</label></form><aside className="activity-preview"><span className="eyebrow">ANTEPRIMA ROSS</span><div className="activity-icon"><Sparkles size={24} /></div><h3>{form.type}</h3><p>Una sessione di {form.duration} minuti su <strong>{form.topic.toLowerCase()}</strong>, partendo dalla memoria “{form.memory}”.</p><div><span>{form.resident}</span><span>{form.level}</span><span>{form.group}</span></div><p className="preview-copy">“Elena, ti andrebbe di riprendere le fotografie del viaggio in Sicilia?”</p></aside></div><div className="modal-actions"><button className="ghost-button" onClick={() => onSchedule(form)}>Programma domani</button><button className="primary-button" onClick={onStart}>Avvia ora <ArrowRight size={16} /></button></div></Modal>;
}

function CalendarStamp({ date }) { const day = date?.slice(-2) || "22"; return <span className="calendar-stamp"><small>SET</small><strong>{day}</strong></span>; }
